...
public class Feitizo implements Comparable<Feitizo> {
    ...
    public static Collection<String> ingredientesUnicos(Collection<Feitizo> feitizos) {
        ...
        for (int i = 0; i < crearFeitizosExemplo().length; i++) {
            for (int j = 0; j < crearFeitizosExemplo()[i].getIngredientes().length; j++) {
                ingredientesUnicos.add(crearFeitizosExemplo()[i].getIngredientes()[j]);
            }
    ...
    @Override
    public int compareTo(Feitizo feitizo) {
        return this.getNome().compareTo(feitizo.getNome());
    }
}
